...
function normalizeSymbol(rawSymbol) {
  ...
  if (["BTC", "BTCUSD", "BTC/USD", "BTC-USD", "BITCOIN"].includes(raw) || raw.includes("比特币")) {
...
function buildRuleReport({ displaySymbol, interval, range, meta, bars, cards, gptHtml, options }) {
  ...
  const levels = [
    ...
    .join("");
  const userOptions = `<section class="section"><h2>用户选择条件</h2><table><tr><th>菜单</th><td>${safeHtml(options.menu)}</td></tr><tr><th>数据范围</th><td>${safeHtml(range)}</td></tr><tr><th>增强模块</th><td>${safeHtml((options.modules || []).join("、") || "默认")}</td></tr><tr><th>额外要求</th><td>${safeHtml(options.extra || "无")}</td></tr></table></section>`;
...
async function callOpenAI(env, payload, model) {
  ...
  const res = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      model: model || env.OPENAI_MODEL || "gpt-5",
      input: [
  ...
  if (!res.ok) {
    ...
    return {
      ...
      html: `<section class="section"><h2>GPT 综合解读</h2><p>GPT 调用失败，已保留规则版报告。错误：${safeHtml(text.slice(0, 500))}</p></section>`,
    };
  }
  ...
  return {
    ...
    html: outputText || '<section class="section"><h2>GPT 综合解读</h2><p>GPT 已调用，但返回内容为空。</p></section>',
  };
}

async function handleReport(request, env) {
  const data = await request.json();
  const { yahoo, display } = normalizeSymbol(data.symbol);
  const interval = data.interval || "60m";
  const range = data.range || "10d";
  const model = data.model || env.OPENAI_MODEL || "gpt-5";
  const { meta, bars } = await fetchYahooBars(yahoo, range, interval);
  ...
  const gpt = await callOpenAI(env, payload, model);
  const reportHtml = buildRuleReport({
  ...
  async fetch(request, env) {
    ...
    try {
      if (url.pathname === "/" || url.pathname === "/health") {
        return jsonResponse({ ok: true, service: "kline-robot-worker" }, 200, env);
      }
