...
| 机制 | 说明 |
|:---|:---|
| **运行锁** | `~/.donew-trader-long/long-term-trader.lock`，内容为 `pid/mode/startedAt`。用 `fs.openSync(lock, 'wx')` 原子创建，只有第一个进程能拿锁，杜绝并发写坏文件。锁失败一律抛错退出；锁仅在同 pid 且未超时（10 分钟）时有效，进程不存在或超时后自动清理。 |
...
| **K线新鲜度前置** | 按 `America/New_York` 交易日判断：美股收盘后必须看到当日或最近应有交易日，周末/节假日单独宽限（超 4 天仍过期）。过期时不生成 BUY、不开仓、离场也跳过自动平仓，dashboard 标注 `旧K线 / 仅供历史查看`。 |
| **数据来源元数据** | K线缓存、信号文件、dashboard 数据写入 `source / fetchedAt / lastDataTime / ok / error / stale / staleReason`。 |

## 版本

v2.0.6 · 2026-08-06 · 锁原子创建 + ET交易日新鲜度 + 离场防旧K线
