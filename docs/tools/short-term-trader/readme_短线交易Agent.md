...
| 标的 | 类型 | 交易时段 |
|:---|:---|:---|
| QQQ | 纳斯达克100 ETF | 美股时段 |
| IBIT | 比特币 ETF | 美股时段 |
| MSTR | MicroStrategy（高波动） | 美股时段 |
| TSLA | 特斯拉 | 美股时段 |
| EEM | 新兴市场 ETF | 美股时段 |
| BTC-USD | 比特币 | **24小时** |
| UGL | 2倍做多黄金 ETF | 美股时段 |
...
- **K线周期**：5分钟
- **运行频率**：5分钟一次（美股时段全标的；休市时仅 24h 标的一直跑）
- **方向**：仅做多（BUY / SKIP）
- **AI 引擎**：DeepSeek Chat（纯K线技术分析，不参考新闻/基本面）
- **入场规则**（技术规则 + AI 评分）：
  - 技术规则 4 项：MACD > 0、RSI > 50、EMA（收盘 > EMA9）、量比 > 0.8
  - **EMA + 量比必须通过（硬条件）**，MACD/RSI 至少过 1 个，总通过 ≥ 3 项
  - 规则通过后再由 AI 评分，评分 ≥ 6 才开仓
- **离场**：ATR 止损止盈自动触发（SL = max(2.5×ATR, 0.4%)，TP = 2×SL），收盘不强制平仓（跨日持有）
- **仓位管理**：每笔 $10,000，同时最多 5 笔（= 50% 总资金上限）
