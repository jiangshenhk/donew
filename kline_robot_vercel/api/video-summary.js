...
## 📋 核心要点
列出${mode === "article" ? "文章" : "视频"}中提到的 3-8 个核心观点或重要信息，每条以"**要点：**"开头单独列出。
...
- ✅ **机会信号**：内容中提及的正面信号、增长机会、有利因素。
- ⚠️ **风险提示**：内容中暗示的风险、隐患、需要警惕的地方。
- 📈 **背景补充**：这个话题相关的背景知识、行业趋势或历史脉络。
- 🔗 **延伸思考**：这个内容可能引发的进一步思考或值得关注的问题。
- 🧠 **独特见解**：基于内容之外的知识，给出你独有的洞察或评价。

注意：所有数字和百分比请带正负号（如 +5.2% 或 -3.1%），正向使用绿色渲染，负向使用红色渲染。
