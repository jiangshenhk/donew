...
function twoBPositionSvg(twoB) {
  if (!twoB?.valid) {
    return `<div class="abc-stage"><svg class="abc-stage-svg" viewBox="0 0 380 120" role="img" aria-label="2B结构未成立"><rect x="8" y="8" width="364" height="104" rx="14" fill="#0b1227" stroke="rgba(255,255,255,.06)"/><rect x="18" y="16" width="344" height="22" rx="11" fill="rgba(255,255,255,.04)" stroke="rgba(255,255,255,.08)"/><text x="28" y="30" fill="#d7e3ff" font-size="10" font-weight="800">2B 结构未成立</text><rect x="34" y="46" width="312" height="50" rx="12" fill="rgba(255,255,255,.025)" stroke="rgba(255,209,102,.22)" stroke-dasharray="6 6"/><text x="48" y="67" fill="#ffd166" font-size="9.6" font-weight="800">当前先不强画 2B 位置图</text><text x="48" y="84" fill="#dbe5ff" font-size="8.8">最近几根K线未形成标准的假突破 / 假跌破后快速回收结构。</text></svg></div>`;
  }
...
function abcStructureSectionHtml({ abc, twoB, bars }) {
  ...
  return `<section class="section structure-section"><h2>ABC 和 2B 结构，AI判断位置</h2><div class="structure-grid"><article class="structure-card"><h3>ABC / 123 主判断</h3><p class="structure-lead">${safeHtml(abc.summary)}</p><div class="structure-meta"><span class="structure-chip">${safeHtml(abc.primaryJudgement || "ABC结构")}</span><span class="structure-chip structure-chip-gold">${safeHtml(abc.currentStage || abc.stage || "结构待确认")}</span></div><div class="structure-meta"><span class="structure-chip structure-chip-soft">${safeHtml(abc.volumeDetail)}</span><span class="structure-chip structure-chip-soft">${safeHtml(abc.strength)}</span></div>${abcPositionSvg(abc)}</article><article class="structure-card"><h3>2B 结构判断</h3><p class="structure-lead">${safeHtml(twoB.summary)}</p>${twoBMeta}${twoBPositionSvg(twoB)}</article></div></section>`;
}
