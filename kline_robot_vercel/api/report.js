...
async function callOpenAI(payload, model) {
  if (!process.env.OPENAI_API_KEY) {
    return {
      used: false,
      message: "Vercel 未配置 OPENAI_API_KEY，已生成规则版报告。",
      html: '<section class="section"><h2>GPT 综合解读</h2><p>Vercel 未配置 OPENAI_API_KEY，本次只生成规则版报告。</p></section>',
    };
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const model = data.model || process.env.OPENAI_MODEL || "gpt-5";
    ...
    const gpt = await callOpenAI(payload, model);
    const html = buildReport({ displaySymbol: display, interval, range, bars, cards, gptHtml: gpt.html, options });
