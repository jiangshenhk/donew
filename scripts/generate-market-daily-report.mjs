...
要求：
1. 不复述新闻，要判断市场正在交易什么。
2. 结论置顶，先给市场阶段、资金流向、风险评分。
3. 必须分析QQQ/QLD、BTC/MSTR、黄金、美元、美债10Y、VIX、半导体、EEM。
4. 卖Put判断必须符合人工日报逻辑：风险高则等待，不追strike，不虚构Delta、IV、权利金或实时价格。
5. 行情缺失时必须明确写“未取到”，不能补猜。
6. 输出Markdown，不增加其他章节。`;
...
async function callAI(){
  ...
  try {
    ...
    catch { throw new Error(`AI endpoint returned non-JSON (${r.status}): ${text.slice(0,160)}`); }
...
fs.writeFileSync(path.join(statusDir,`latest-${reportType}.json`),JSON.stringify({
  status:'success',
  reportType,
