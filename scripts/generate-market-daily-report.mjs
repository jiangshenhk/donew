...
const prompt = `你是服务卖Put投资者的跨资产市场分析师。严格按照历史《市场结构日报》格式生成${label}。必须保持：# 📊 市场结构日报、一句话结论、策略矩阵、资金联动、宏观资产观察、风险资产表现、驱动拆解、卖Put策略、今日动作、最后的话。${reportType==='morning'?'重点分析隔夜美股收盘和今日交易准备。':'重点分析亚洲欧洲交易变化和今晚美股准备。'}不要复述新闻，要判断市场正在交易什么；不要虚构期权数据。\n新闻：${JSON.stringify(news)}\n行情：${JSON.stringify(pricePayload).slice(0,8000)}`;
