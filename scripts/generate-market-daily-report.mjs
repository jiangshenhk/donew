...
要求：不复述新闻，要判断市场交易逻辑；卖Put判断必须符合人工日报逻辑；不要虚构期权数据。
输出Markdown。
...
fs.writeFileSync(path.join(outputDir,'data',`latest-${reportType}.json`),JSON.stringify({status:'success',reportType,generatedAt:new Date().toISOString(),newsCount:news.length},null,2));
