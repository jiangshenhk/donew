...
const prompt = `你是服务卖Put投资者的跨资产市场分析师。严格按照历史《市场结构日报》格式生成${label}。保持章节：# 📊 市场结构日报、一句话结论、策略矩阵、资金联动、宏观资产观察、风险资产表现、驱动拆解、卖Put策略、今日动作、最后的话。${reportType==='morning'?'重点分析隔夜美股收盘和今日交易准备。':'重点分析亚洲欧洲交易变化和今晚美股准备。'}
...
function updateHistory(markdown){
 ...
 const row=`| **${displayDate}** | ${weekday} | 日报 | [📊 ${label}](/docs/市场/${hkDate}市场结构日报(${label}).md) | ${core} | ${action} |`;
 if(text.includes(`[📊 ${label}](/docs/市场/${hkDate}市场结构日报(${label}).md)`)) return;
