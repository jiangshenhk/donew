...
export function sanitizeReport(text) {
  ...
  value = value.replace(/保证盈利/g, '可能盈利');
  value = value.replace(/必涨/g, '可能上涨');
  value = value.replace(/必跌/g, '可能下跌');
  value = value.replace(/无风险(?!资产|利率)/g, (match, offset) => {
    const before = value.slice(Math.max(0, offset - 6), offset);
    return /并非|不是|绝非|并非完全/.test(before) ? match : '低风险';
  });
  return value;
}
